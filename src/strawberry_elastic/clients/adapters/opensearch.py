"""Version-agnostic OpenSearch adapter."""
...
class OpenSearchAdapter(BaseElasticAdapter):
    """
    Version-agnostic adapter for OpenSearch clients.

    Supports both sync and async OpenSearch clients across versions 1.x and 2.x.
    ...
    async def _detect_capabilities(self) -> None:
        """Detect OpenSearch capabilities at runtime."""
        self._capabilities = {
            "supports_pit": False,
        ...
        try:
            ...
            # Point in Time support (OpenSearch 2.0+)
            if major_version >= 2:
                self._capabilities["supports_pit"] = True

            # Async search support (OpenSearch 1.0+)
